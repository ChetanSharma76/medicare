...
const MyAppointments = () => {
  ...
  const getUserAppointments = async () => {
    try {
      ...
      if(data.success){
        setAppointments(data.appointments.reverse()) //in order to place new appointment or recent appointment at top we reverses the array
      }
    } catch (error) {
      console.log(error)
          ...
          appointments.map((item,index)=>(
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
              ...
              <div className='flex flex-col gap-2 justify-end'>
                ...
                {item.isCompleted && <button className='sm:min-w-48 py-1 text-center border border-green-500 rounded text-green-500' >Completed</button>}
              </div>
            </div>
          ))
        }
